...
image_filename <- "Resized_image.png"   # <-- CHANGE THIS to your image file name
...
results_dir <- file.path(root_dir, "Results", "R", "NEWrpautratSuperpoint")
...
########################################
print_array_or_tensor <- function(x, name = "Object") {
  if (inherits(x, "torch_tensor")) {
    cat(name, "(torch tensor):\n")
    cat("  shape:", x$size(), "\n")
    cat("  min:", sprintf("%.6f", x$min()$item()), 
        "mean:", sprintf("%.6f", x$mean()$item()), 
        "max:", sprintf("%.6f", x$max()$item()), "\n")
  } else if (is.numeric(x) && is.array(x)) {
    cat(name, "(R array):\n")
    cat("  dim:", dim(x), "\n")
    cat("  min:", sprintf("%.6f", min(x)), 
        "mean:", sprintf("%.6f", mean(x)), 
        "max:", sprintf("%.6f", max(x)), "\n")
  } else if (is.numeric(x) && !is.array(x)) {
    cat(name, "(numeric vector):\n")
    cat("  length:", length(x), "\n")
    cat("  min:", sprintf("%.6f", min(x)), 
        "mean:", sprintf("%.6f", mean(x)), 
        "max:", sprintf("%.6f", max(x)), "\n")
  } else {
    cat(name, "is an unknown type:", class(x)[1], "\n")
  }
  invisible(x)
}
# ============================================
...
# ============================================
process_image_safe <- function(image_path, model) {
  total_start <- Sys.time()
  
  t1 <- Sys.time()
  img <- image_read(image_path)
  print_array_or_tensor(as.numeric(img[[1]]), "image processed by python:")
  
  img_array <- as.numeric(img[[1]])  
  t3 <- Sys.time()
  
  print_array_or_tensor(img_array, "after converting to array & normalizing")
  
  img_tensor <- torch_tensor(img_array)$view(c(1, 1, nrow(img_array), ncol(img_array)))
  t4 <- Sys.time()
  
  print_array_or_tensor(img_tensor, "after converting to tensor")
  img_info <- image_info(img)
  final_h <- img_info$height
  final_w <- img_info$width
  gc()
...
gc()
start_time <- Sys.time()
result <- process_image_safe(image_path, model)
